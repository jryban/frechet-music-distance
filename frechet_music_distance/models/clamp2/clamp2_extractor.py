...
class CLaMP2Extractor:
    def __init__(self) -> None:
        ...
        self.model = CLaMP2Model(
            m3_config, text_model_name=config.TEXT_MODEL_NAME, hidden_size=config.CLAMP2_HIDDEN_SIZE
        )
    ...
    def extract_feature(self, data: str) -> NDArray:
        ...
        for input_segment in segment_list:
            ...
            pad_indices = (
                torch.ones((config.PATCH_LENGTH - input_segment.size(0), config.PATCH_SIZE)).long()
            ...
            last_hidden_states = self.model.get_music_features(
                music_inputs=input_segment.unsqueeze(0).to(self.device),
        ...
        if remain_chunk_len == 0:
            ...
            feature_weights = torch.tensor(
                [max_input_length] * full_chunk_cnt + [remain_chunk_len], device=self.device
            ).view(-1, 1)
