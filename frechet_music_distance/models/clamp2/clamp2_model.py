...
class CLaMP2Model(PreTrainedModel):
    def __init__(
        self, music_config: BertConfig, text_model_name: str = TEXT_MODEL_NAME, hidden_size: int = CLAMP2_HIDDEN_SIZE
    ) -> None:
    ...
    def get_music_features(self, music_inputs: torch.Tensor, music_masks: torch.Tensor) -> torch.Tensor:
        model_after_inference = self.music_model(music_inputs.to(self.device), music_masks.to(self.device))
        music_features = model_after_inference["last_hidden_state"]
