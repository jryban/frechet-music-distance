...
class CLaMP2(PreTrainedModel):
    ...
    def __init__(
        ...
        self._text_model = AutoModel.from_pretrained(text_model_name) # Load the text model
        self._text_proj = torch.nn.Linear(self._text_model.config.hidden_size, hidden_size) # Linear layer for text projections
        torch.nn.init.normal_(self._text_proj.weight, std=0.02) # Initialize weights with normal distribution

        self._music_model = M3PatchEncoder(music_config) # Initialize the music model
        self._music_proj = torch.nn.Linear(M3_HIDDEN_SIZE, hidden_size) # Linear layer for music projections
        torch.nn.init.normal_(self._music_proj.weight, std=0.02) # Initialize weights with normal distribution
    ...
    def get_music_features(self, music_inputs: torch.Tensor, music_masks: torch.Tensor) -> torch.Tensor:
        music_features = self._music_model(music_inputs.to(self.device), music_masks.to(self.device))["last_hidden_state"]

        # Normalize features (Reduce Temporal Dimension)
        music_features = self.avg_pooling(music_features, music_masks)
        music_features = self._music_proj(music_features)
